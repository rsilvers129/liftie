...
/*
 * Point puppeteer at the browser bundled with the checkout.
 *
 * .puppeteerrc.cjs covers the install, but it is resolved relative to the
 * working directory, and the app is started from the parent of the repo. Set
 * the cache directory from this module's own location so the browser is found
 * no matter where node was launched from.
 */
process.env.PUPPETEER_CACHE_DIR ??= join(import.meta.dirname, '..', '..', '.cache', 'puppeteer');
