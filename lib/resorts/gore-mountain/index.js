...
const debug = Debug('liftie:resort:gore-mountain');

// Use a real Chrome User-Agent to bypass ORDA blocking
const chromeUserAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';

const goreUrl = 'https://goremountain.com/the-mountain/conditions/';
...
// Rate limiting: 15-60 minute random intervals
const MIN_FETCH_INTERVAL = 15 * 60 * 1000;  // 15 minutes
const MAX_FETCH_INTERVAL = 60 * 60 * 1000;  // 60 minutes

// Operating hours in NY timezone (EST/EDT)
// Gore Mountain: typically 8:30 AM - 4:00 PM
const OPEN_HOUR = 8;
    ...
    fetch(goreUrl, {
        ...
        .then(async res => {
            ...
            if (res.status < 200 || res.status >= 300) {
                debug('Gore Mountain fetch error: %d', res.status);
                return fn(null, cachedStatus);
            }
            ...
            debug('Gore Mountain Lift Status (via Chrome UA):', liftStatus);
            fn(null, liftStatus);
        })
        .catch(err => {
            ...
            debug('Gore Mountain fetch exception:', err);
