...
async function fetchWithPuppeteer() {
  ...
  try {
    ...
    debug('Navigating to %s', goreUrl);
    await page.goto(goreUrl, { waitUntil: 'networkidle2', timeout: 60000 });
    debug('Waiting for lift data');
    await page.waitForSelector('.lifts-row', { timeout: 30000 });
    const html = await page.content();
    await browser.close();
    debug('Got page content');
    return html;
  } catch (err) {
...
export default function parse(dom) {
  ...
  return fetchWithPuppeteer()
    .then(html => {
      ...
      debug('Gore Mountain Lift Status (via Puppeteer):', liftStatus);
      return liftStatus;
    })
    .catch(err => {
      lastFetchTime = Date.now();
